...
class RelationalGraphLayer(nn.Module):
    ...
    def forward(self, A, batch_nodes, mode):
        if mode == "report":
            ...
            new_emb = self.aggregate_code_two_order(
                mask_1_code, mask_2_code_report, mask_2_code_code,
                unique_code_neighs_1, unique_code_report_neighs_2, unique_code_code_neighs_2, False)
            return new_emb

        elif mode == "code":
            # code-report和code-code
            mask_1_code, mask_2_code_report, mask_2_code_code, \
            unique_code_neighs_1, unique_code_report_neighs_2, unique_code_code_neighs_2, \
            ...
            # Aggregate second-order neighbors and update first-order neighbors
            new_code_emb = self.aggregate_code_two_order(
                mask_1_code, mask_2_code_report, mask_2_code_code,
                unique_code_neighs_1, unique_code_report_neighs_2, unique_code_code_neighs_2, False)
            new_report_emb = self.aggregate_report_two_order(
                mask_1_report, mask_2_report_code, unique_report_neighs_1, unique_report_code_neighs_2, False)
    ...
    def aggregate_code_two_order(self, np_mask_1_code, np_mask_2_report, np_mask_2_code, unique_code_neighs_1, unique_code_report_neighs_2, unique_code_code_neighs_2, weighted):
        if weighted:
            mask_1_code = torch.tensor(np_mask_1_code, dtype=torch.float32)
            mask_2_report = torch.tensor(np_mask_2_report, dtype=torch.float32)
            mask_2_code = torch.tensor(np_mask_2_code, dtype=torch.float32)
        else:
    ...
    def aggregate_report_two_order(self, np_mask_1_report, np_mask_2_code, unique_report_neighs_1, unique_report_code_neighs_2, weighted):
        if weighted:
            mask_1_report = torch.tensor(np_mask_1_report, dtype=torch.float32)
            mask_2_code = torch.tensor(np_mask_2_code, dtype=torch.float32)
        else:
